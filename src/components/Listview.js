...
export default class CustomListView extends React.Component {
  constructor() {
    super();
    this.state = { dungeonsData, selected: 1 };
  }
  ...
  render() {
    return (
      ...
      >
        ...
        {this.state.dungeonsData.map(dungeonSeries => {
          return (
            <div>
              <ListViewSection
                header={this.renderSectionHeader(dungeonSeries.dungeonSeries)}
              >
                {dungeonSeries.dungeons.map((dungeon, idx) => {
                  return this.renderItem(
                    dungeonSeries.dungeonSeries + idx,
                    dungeon.dungeonName
                  );
                })}
              </ListViewSection>
